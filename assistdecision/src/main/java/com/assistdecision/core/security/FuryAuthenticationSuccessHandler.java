package com.assistdecision.core.security;

import com.assistdecision.config.RedisComponent;
import com.assistdecision.domain.output.ActionOutput;
import com.assistdecision.domain.output.LoginOutput;
...
import com.assistdecision.service.ActionService;
import com.assistdecision.service.MenuService;
...
//        SecurityContextHolder securityContextHolder = new SecurityContextHolder();
        ...
        menus = menus.stream().filter(MenuOutput -> MenuOutput.getDisPlayOrderBy() != null && MenuOutput.getDisPlayOrderBy() >= 1000 ).sorted(Comparator.comparingInt(MenuOutput::getDisPlayOrderBy)).collect(toList());
    ...
    private Map<String, List<String>> getActionOutput(List<ActionOutput> actions) {
        ...
        List<Action> action = actions.stream().filter(Action -> Action.getCode().indexOf("A_ASSESSMENT") >= 0).collect(toList());
