...
@ApiModel(value = "AppraisalTemplate考核模板", description = "考核模板")
public class AppraisalTemplate implements Serializable {
    @Id
    @Column(length = 6)
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "appraisalTemplateGenerator")
    @SequenceGenerator(name = "appraisalTemplateGenerator", sequenceName = "appraisalTemplate_sequence",allocationSize = 1,initialValue = 1)
    @ApiModelProperty(value="考核模板id,后台生成无法修改",name="id")
    private Integer id;

    @NotBlank
    @Length(min=1, max=255, message="模板名称长度只能在{min}和{max}之间")
    @Column(nullable = false,length = 255)
    @ApiModelProperty(value="模板名称,长度最大255",name="name",required=true)
    private String name;

    @Range(min=0, max=99, message="模板类型值只能在{min}和{max}之间")
    @Column(nullable = false, length = 2)
    @ApiModelProperty(value="模板类型",name="type")
    private Integer type;

    @Range(min=0, max=99, message="对象类型值只能在{min}和{max}之间")
    @Column(nullable = false, length = 2)
    @ApiModelProperty(value="对象类型",name="objectType")
