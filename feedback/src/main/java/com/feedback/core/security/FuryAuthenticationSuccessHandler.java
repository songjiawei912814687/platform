...
import com.feedback.domain.output.LoginOutput;
...
import com.feedback.service.ActionService;
...
public class FuryAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    private Map<String, List<String>> getActionOutput(List<ActionOutput> actions) {
        ...
        List<Action> action = actions.stream().filter(Action -> Action.getCode().indexOf("A_FEEDBACK") >= 0).collect(toList());
